...
const routes={'/assets/agency-planner-logo.png':'assets/agency-planner-logo.png','/':'index.html','/index.html':'index.html','/app.html':'app.html','/assets/workspace.js':'assets/workspace.js','/assets/workspace.css':'assets/workspace.css'};
http.createServer((req, res) => {
  const route = new URL(req.url, 'http://localhost').pathname;
  if (!routes[route]) {
    res.writeHead(404); return res.end('Not found');
  }
  const type=route.endsWith('.png')?'image/png':route.endsWith('.js')?'application/javascript':route.endsWith('.css')?'text/css':'text/html';
